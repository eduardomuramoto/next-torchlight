...
export default function VolunteerFormSection() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      const response = await fetch("/api/volunteerEmail", {
        method: "POST",
        headers: {
  ...
  return (
    <section className="w-full bg-lightBackground flex gap-20 p-16 lg:px-20" id="enquiry">
      <div className="w-full">
        <h3 className="font-teko text-6xl uppercase font-semibold pb-6 text-background">Get Involved!</h3>
        <p className="font-teko text-6xl uppercase font-semibold pb-6 text-background">Make a difference.</p>
        <form onSubmit={handleSubmit} className="flex flex-wrap lg:flex-nowrap lg:gap-20 w-full">
          ...
          <div className="w-full lg:w-2/3 h-full ">
            ...
            <BlueButton className="ml-auto" label="Let's connect" afterIcon="yes" isSubmit={true} />
          </div>
        </form>
        {/* Status Message */}
